...
# =============================================================================
    ...
    def bigger_fig(self):
        ...
        self.default_figsize[0] += 2
        self.default_figsize[1] += 1
        ...
        for row in range(self.style_array.shape[0]-1):
                    ...
                    if self.style_array[row, index] == 'n':
                        ...
                        if direction == 'right':
                            ...
                            if self.style_array[row-1, index] == 'f' or self.style_array[row-1, index] == 'p':
                                ...
                                line_segment = np.append(line_segment, temp_arr[0, -1])
                                distance_segment = np.append(distance_segment, temp_arr[-1, -1])
                        ...
                        if self.style_array[row-1, index] == 'f' or self.style_array[row-1, index] == 'p':
                            ...
                            if above_direction == 'right':
                                ...
                                line_segment = np.append(temp_arr[0], temp_arr[-1])
                                distance_segment = np.append(temp_arr_distance[0], temp_arr_distance[-1])
                            ...
                            if above_right_direction == 'left' or above_right_direction == 'both':
                                ...
                                second_line_segment = np.array([temp_arr[0], self.formation_polygons[row][0, right_well_index]])
                                second_distance_segment = np.array([temp_arr_distance[0], self.formation_polygons[row][-1, right_well_index]])
                                second_line_stack = np.vstack((second_line_segment, second_distance_segment))
                                #Add that line segment to the approriate list
                                if linestyle == '-':
                                    self.dashed_contacts.append(second_line_stack)
            ...
            ###########################################################################################################################        
                    ...
                    if self.style_array[row, index] == 'n':
                        ...
                        if index == len(self.core_or_cuttings)-2: #Adresses the last borehole
                            mask = (self.formation_polygons[row][-1] >= self.locations[index]) & (self.formation_polygons[row][-1] <= self.locations[index+1])
                            line_segment = self.formation_polygons[row][0][mask]
                            distance_segment = self.formation_polygons[row][-1][mask]
                        else:
...
# =============================================================================
    ...
    def create_plot (self):
        ...
        tallest_borehole = np.max(self.well_elev)
        deepest_borehole = min(self.formations_array[-1])
        ...
        for n in range(len(self.w_num)):
            ax.vlines(self.locations[n], color='k', ymin=self.formations_array[-1, n], ymax=self.well_elev[n])
            ax.annotate("W-" + str(self.w_num[n]), (self.locations[n] - self.locations[-1]*0.01 , tallest_borehole + 80)) #Note that this uses 1% of the total length to offset labels over well lines
        
        #Plots the contact lines
        for line in self.solid_contacts:
            ax.plot(line[1], line[0], linestyle='-', color='k')
        ...
        ax.set_ylim(deepest_borehole-10, tallest_borehole+100)
      ...
      ###############################################################################################################################################################          
                ...
                if np.any(self.style_array[row] == 'c'):
                    ...
                    for group in connect_groups.keys():
                        insert_id = connect_groups[group][0]
                        ...
                        total_stack = np.delete(total_stack, connect_groups[group], axis=1)
                        
                        temporary_connection_array = np.array([[],[],[]])
                    
                        for connection in connect_groups[group]:
                            connection_point = self.locations[connection] * slope + y_intercept
                            
                            if left_elev < self.formations_array[-1, connection] and right_elev < self.formations_array[-1, connection]:
                                next_row = 1 
                                while self.style_array[row + next_row, left_index] != 'x':
                                    next_row += 1
                                left_thickness = self.formations_array[row, left_index] - self.formations_array[row + next_row, left_index]
                                next_row = 1 
                                while self.style_array[row + next_row, right_index] != 'x':
                                    next_row += 1
                                right_thickness = self.formations_array[row, right_index] - self.formations_array[row + next_row, right_index]
                                
                                if left_thickness >= right_thickness:
                                    connection_point_bottom = connection_point - left_thickness
                                else:
                                    connection_point_bottom = connection_point - right_thickness
                                
                            else:    
                                connection_point_bottom = self.formations_array[-1, connection]
                            
                            point_array = np.array([[connection_point], [connection_point_bottom], [self.locations[connection]]])
                            
                            temporary_connection_array = np.hstack((temporary_connection_array, point_array ))
                        
                        insert_groups[insert_id] = temporary_connection_array
                        
                    for insert_index in insert_groups.keys():
                        total_stack = np.insert(total_stack, [insert_index], insert_groups[insert_index], axis=1)         
