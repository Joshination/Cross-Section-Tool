...
#############################################################################################################
    ...
    def create_contact_line_array(self):
        pass
...
#####################################################################################################################################################        
    ...
    def create_plot (self):
        ...
        for n in range(len(self.w_num)):
            ...
            ax.annotate("W-" + str(self.w_num[n]), (self.locations[n] - self.locations[-1]*0.01 , tallest_borehole * 1.2)) #Note that this uses 1% of the total length to offset labels over well lines
        
        for polygon in self.formation_polygons:
            ax.plot(polygon[-1], polygon[0], color='k', linewidth=1)
